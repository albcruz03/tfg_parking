...
url = 'http://frostpeak.ddns.net:4747/cam/1/frame.jpg'
...
os.makedirs('imagenes_normales', exist_ok=True)
...
def capture_and_process_image():
    ...
    if response.status_code == 200:
        ...
        normal_image_path = os.path.join('imagenes_normales', filename)
        processed_image_path = os.path.join('imagenes_procesadas', 'processed_' + filename)

        # Guarda la imagen en el directorio de imágenes normales
        with open(normal_image_path, 'wb') as f:
            f.write(response.content)

        print(f'Imagen guardada como {normal_image_path}')
        
        # Leer la imagen
        frame = cv2.imread(normal_image_path)
        ...
        # Guardar la imagen resultante en el directorio de imágenes procesadas
